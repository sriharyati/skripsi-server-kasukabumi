...
module.exports = {
      ...
      testimonialPage:  async (req, res) => {
        try {
            const testimony = await Testimonial.find()
              .select('_id photoTestimonial name rate content familyName familyOccupation')
              .limit(50)
    ...
    formPage: async (req, res) => {
      ...
      if ( 
        ...
        familyOccupation === undefined) {
        return res.status(404).json({ message: "Lengkapi semua field" });
      }

      const testimonial = await Testimonial.create({
        photoTestimonial: `images/${req.file.filename}`,
        name,
        rate,
        content,
        familyName,
      ...
      return res.status(201).json({ message: "Success Testimonial", testimonial });
    },
